...
git-hygiene-remediate:
	$(PYTHON) scripts/remediate_git_hygiene.py --root $(ROOT) --repo $(ROOT) --repo ../orxaq --stale-days 1 --max-remote-deletes 200 --max-local-deletes 200 --apply --output $(ROOT)/artifacts/autonomy/git_hygiene_remediation.json --json
...
swarm-cycle-report: swarm-health-snapshot t1-basic-model-policy-check privilege-policy-check pr-approval-remediate git-delivery-policy-check git-hygiene-remediate git-hygiene-check backend-upgrade-policy-check api-interop-policy-check backlog-control-once
	-$(PYTHON) scripts/swarm_todo_health_current.py --root $(ROOT) --json
